...
5. **帮助文档面板**
   - Caddy 二进制下载指引（Mac/Windows 分开）
   - 完整小白操作步骤
   - 风险声明，明确区分等级 1、等级 2 使用场景

> 重要：**所有输出仅仅是字符串文本，交给用户复制，插件不会写文件、不会启动程序**。

## UI 面板结构（折叠面板 · 层层递进）

设置卡片采用**分层折叠面板**：最外层 4 个模块一次只展开一个（头部带实时状态徽章，不展开即可总览配置状态）；公网安全配置内部再按步骤折叠，生成哈希后自动展开下一步。

```
# DSH 远程访问鉴权（设置卡片）

## 外层折叠面板（一次展开一个，头部显示状态徽章）
▸ 🔐 页面访问密码【安全等级1｜内网模式】   ← 默认展开 · 徽章：页面锁已启用/未启用
│   ⚠️警告：仅保护Web页面，/api /ws接口未防护，禁止公网穿透
│   [输入框：设置页面密码] [保存密码]
│   [清除本地授权token（测试用）] [关闭页面锁]
▸ 🛡️ 公网安全配置【安全等级2｜需要Caddy反向代理】  · 徽章：哈希已生成/待生成哈希
│   └─ 内层步骤折叠（生成哈希后自动展开下一步）
│      ① 生成BCrypt密码哈希（前端本地计算）→ [复制哈希]
│      ② Caddy配置参数（端口/后端/账号）→ [生成完整Caddyfile]
│      ③ 生成的Caddyfile + Mac/Windows启动命令 → [一键复制]
▸ ⚕️ 安全风险审计  · 徽章：审计正常 / N 项高危 / 待审计
▸ 📖 小白使用指引
```
